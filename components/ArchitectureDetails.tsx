...
export default function ArchitectureDetails({
  ...
  return (
    <>
      {/* Summary */}
      <p className="font-sans text-sm text-body leading-relaxed mb-6 bg-canvas/50 p-4 rounded-lg border border-border/30">
      ...
      <div className="mb-6">
        ...
        <div className="space-y-2">
          {architecture.flow.map((step, idx) => (
            <div
              key={idx}
              className="flex items-center gap-3 p-2.5 rounded-md bg-canvas/80 border border-border/30 text-xs font-mono"
            >
      ...
      <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {architecture.components.map((comp, idx) => (
            <div
              key={idx}
              className="p-3.5 rounded-lg bg-canvas/60 border border-border/40 hover:border-primary/30 transition-colors"
            >
